...
const DialogForm: React.FC<IDialogFormProps> = ({ isOpen, onClose, currentDate, accountId }) => {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50" onClick={handleCloseDialog}>
      <div
        className="relative p-4 w-full max-w-lg max-h-full bg-white rounded-lg shadow"
        onClick={(e) => {
          e.stopPropagation();
        }}
      >
        ...
        <div className="my-10 px-4">
            ...
            <div className="space-y-4">
              <div className="flex items-start">
                ...
                <div className="w-full">
                  <input {...register("noOfBug")} placeholder="Enter no. of bug" className={cn("w-full border outline-none px-4 py-2 rounded-md")} />
                  {errors.noOfBug && (
                    <p className="flex items-center  gap-1 text-red-500 text-sm">
                      <Warning />
                      {errors.noOfBug.message}
                    </p>
                  )}
                </div>
              </div>
              ...
              <div className="flex items-start">
                ...
                <div className="w-full">
                  <input {...register("token")} placeholder="Enter the token" className={cn("w-full border outline-none px-4 py-2 rounded-md")} />
                  {errors.token && (
                    <p className="flex items-center  gap-1 text-red-500 text-sm">
                      <Warning /> {errors.token.message}
                    </p>
                  )}
                </div>
              </div>
              ...
              <div className="flex items-start">
                <label className="w-[200px] text-nowrap">Comment</label>
                <div className="w-full">
                  <textarea {...register("comment")} placeholder="Enter comment" className={cn("w-full border outline-none px-4 py-2 rounded-md")} />
                  {submissionError && (
                    <p className="flex items-center  gap-1 text-red-500 text-sm">
          ...
          <div className="flex gap-4 justify-end mt-8">
            <Button onClick={handleCloseDialog} variant="outline" className="font-normal focus:outline-[0px]">
              Cancel
            </Button>
            <Button
              aria-label="create"

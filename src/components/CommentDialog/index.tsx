...
const CommentDialog: React.FC<ICommentDialogProps> = ({ commentAdded, isOpen, onClose, currentDate, accountId }) => {
  ...
  const handleComment = async (data: IFormData): Promise<void> => {
    const res = await axios.post(`${TEMP_BACKEND_URI}/users/comment/${accountId}/${currentDate}`, {
      ...
      >
        ...
        <div className="my-10 px-4">
                ...
                <div className="w-full">
                  ...
                  {errors && errors?.comment && (
                    <p className="flex items-center  gap-1 text-red-500 text-sm">
                      <Warning /> {errors?.comment?.message}
                    </p>
                  )}
                  {submissionError && (
                    <p className="flex items-center  gap-1 text-red-500 text-sm">
          ...
          <div className="flex gap-4 justify-end mt-8">
            <Button data-testid="Cancel" onClick={handleCloseDialog} variant="outline" className="font-normal focus:outline-[0px]">
              Cancel
            </Button>
            <Button
              data-testid="post"

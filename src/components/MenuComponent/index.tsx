    ...
    <>
      <div className="mt-6 md:hidden">
        <Button
          ...
          variant={"ghost"}
          prefixIcon="List"
          prefixIconColor="#1D1B20"
          className="px-0 focus:outline-[0px]"
        />
      </div>
      <div
        className={cn(
          "max-w-[280px] fixed left-0 top-0 pl-8 z-50 h-screen w-full flex grow flex-col overflow-y-auto bg-[#FAFBFC] transition-transform duration-150 ease-in-out transform md:transform-none",
      ...
      >
        <div className="mt-6 flex items-center justify-between">
          <Link href={"/member-list?page=1"}>
            <ImageComponent src={"/logo/6senseLogo.svg"} alt={"Pattern50 Logo"} width="w-[132px]" height="h-[40px]" className="text-left" />
          </Link>
          <Button
            onClick={() => {
              setMenuIsOpen(false);
            ...
            variant={"ghost"}
            prefixIcon="close"
            prefixIconColor="#323539"
            className="md:hidden focus:outline-[0px]"
          />
        </div>
                      ...
                      {item.type === "link" ? (
                        <li key={item.name}>
                          <Link
                            href={item.href}
                            className={cn(
                              "group items-center text-menuTextColor flex gap-x-3 rounded-md px-3 py-2 text-sm leading-6 font-regular hover:bg-secondary",

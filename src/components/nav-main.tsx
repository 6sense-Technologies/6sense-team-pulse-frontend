...
export function NavMain({
  ...
  const activeItems = ['Dashboard', 'Projects', 'Team', 'Time Log'];

  return (
    <SidebarGroup>
      <SidebarGroupLabel className='text-sidebarSecondaryColor'>
        Platform
      </SidebarGroupLabel>
      <SidebarMenu>
        {items.map((item) => (
            ...
            <Link href={activeItems.includes(item.title) ? item.url : '#'}>
              <SidebarMenuButton
                className={cn('hover:bg-primary hover:text-white', {
                  ...
                  'cursor-not-allowed text-gray-400': !activeItems.includes(
                    item.title
                  ),
                })}
                tooltip={item.title}
                onClick={() =>
                  activeItems.includes(item.title) && onItemClick(item.title)
                }
              >
                {item.icon && (
                  <item.icon

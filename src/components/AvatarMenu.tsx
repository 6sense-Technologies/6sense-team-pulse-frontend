...
const AvatarMenu = () => {
  ...
  const handleClickOutside = (event: MouseEvent) => {
    if (event.target instanceof Node && !(event.target as Element).closest(".avatar-menu-content")) {
      setIsMenuOpen(false);
    }
  };
    ...
    <div className="relative">
      <div onClick={handleAvatarClick} className="cursor-pointer">
        <Avatar className="w-8 h-8">
          ...
          <AvatarFallback className="bg-primary text-white">{session?.user?.name ? getInitials(session.user.name) : "NA"}</AvatarFallback>
        </Avatar>
      </div>
      {isMenuOpen && (
        <div className="absolute right-0 mt-2 w-48 bg-white border border-gray-200 rounded shadow-lg z-10 avatar-menu-content">
          <div className="flex items-center justify-between px-4 py-2">
            <div className="flex flex-col">
              ...
              <p className="text-twelve text-miniSubheadingColor font-normal">{session?.role ? capitalizeFirstLetter(session.role) : "Admin"}</p>
            </div>

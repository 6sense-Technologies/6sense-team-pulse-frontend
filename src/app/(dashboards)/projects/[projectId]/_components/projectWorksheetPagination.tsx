...
const CustomPagination: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  return <div>{children}</div>;
};

export function ProjectWorksheetPagination({ projectId, currentPage, totalPage, onPageChange }: PaginationProps) {
  ...
  const getPagination = (): (number | string)[] => {
    ...
    let endPage = Math.min(totalPage, currentPage + Math.floor(maxPagesToShow / 2));
          ...
          <PaginationPrevious
            ...
            onClick={(e) => {
              ...
              if (currentPage > 1) {
                ...
                router.push(`/projects/${projectId}?tab=worksheet&page=${currentPage - 1}`);
              }
            }}
          />
        </PaginationItem>
            ...
            {typeof page === "number" ? (
              <PaginationLink
                ...
                onClick={(e) => {
                  e.preventDefault();
                  onPageChange(page);
                  router.push(`/projects/${projectId}?tab=worksheet&page=${page}`);
                }}
              >
                {page}
              </PaginationLink>

...
const ProjectCreate = () => {
  ...
  const handleAddtools = (e: React.MouseEvent<HTMLButtonElement, MouseEvent>): void => {
    e.preventDefault();
  ...
  const onSubmit = (data: ProjectTools) => {
    if (validate(data)) {
      const filteredData = {
        ...data,
        tools: data.tools.filter((tool) => tool.toolName.trim() !== "" && tool.toolUrl.trim() !== ""),
      };
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <div className="w-full px-4 pb-8 pt-8 lg:pl-[24px] lg:pr-[14px]">
          ...
          <div className="flex items-center justify-between">
            <GlobalBreadCrumb initialData="Projects" initalLink="/projects" secondayData="Create Project" secondayLink="/projects/create" />
            <span className="hidden pr-2 md:flex">
              <AvatarMenu />
            </span>
          </div>
          <PageHeading title="Create Project" className="pl-[7px] pt-1 lg:pl-[5px]" />
          ...
          <div className="flex w-full max-w-[872px] flex-col items-start lg:flex-row lg:items-center lg:justify-between lg:pt-4">
            <div className="py-4 pl-[6px] lg:pt-0">
              <h1 className="text-[16px] font-semibold lg:pb-2 lg:text-headingXXS">Project Info</h1>
            </div>
          ...
          <div className="mt-8 flex w-full flex-col items-start lg:mt-5 lg:max-w-[872px] lg:flex-row lg:items-center lg:justify-between">
            <div className="pl-2 pt-3 lg:pl-[6px] lg:pt-4">
              <h1 className="pb-1 text-headingXXS font-semibold">Project Management Tool</h1>
            </div>
            <ToolDropdown control={control} name="tools[0].toolName" placeholder="Select" errors={errors.tools?.[0]?.toolName?.message} index={0} />
          </div>
          <div className="flex h-full w-full gap-x-[6px] pl-2 pt-3 lg:pl-[320px]">
            {/* Hide WorkspaceURL if toolName is 'linear' */}
            {isToolName.tools[0].toolName?.toLowerCase() !== "linear" && (
              <WorkspaceURL control={control} name="tools[0].toolUrl" errors={errors.tools?.[0]?.toolUrl?.message} index={0} />
            )}
            {isToolName.tools[0].toolName !== "" && isToolName.tools[0].toolName?.toLowerCase() !== "linear" ? (
          ...
          {fields.slice(1).map((field, index) => (
            <div key={field.id} className="h-full w-full flex-col items-center pl-1 pt-6 lg:pl-[320px]">
              ...
              <div className="flex items-center gap-2 pl-2 pt-4 lg:pl-0 lg:pt-0">
                {/* Hide WorkspaceURL if toolName is 'linear' */}
                {isToolName.tools[index + 1].toolName?.toLowerCase() !== "linear" && (
                  <WorkspaceURL
                    control={control}
                ...
                {isToolName.tools[index + 1].toolName !== "" && isToolName.tools[index + 1].toolName?.toLowerCase() !== "linear" ? (
                  <div className="relative mt-[62px] h-9 w-10 rounded-lg lg:mt-[71px]">
                    <Info
          ...
          <div className="mb-10 ml-2 mt-14 flex-col items-start lg:ml-[320px] lg:items-center">
            <div className="w-full">
              <Button variant="extralight" size="xsExtended" onClick={handleAddtools} className="mb-6 md:max-w-[90px]">
                Add Tool
              </Button>
            </div>
            <div className="w-full">
              <Button variant="darkish" size="lgExtended" type="submit" className="mt-3 w-full md:max-w-[128px]">
                {projectMutation.isPending ? <Circle className="animate-spin" size={14} /> : "Create Project"}
              </Button>
            </div>
          </div>
        </div>
        ...
        <Modal toolName={selectedTool}>
          <div>
            ...
            <div className="mb-3 mt-8 flex justify-end">
              <Button variant="lightex" size="xsExtended" onClick={() => setIsModalOpen(false)}>
                Close
              </Button>
            </div>
          </div>

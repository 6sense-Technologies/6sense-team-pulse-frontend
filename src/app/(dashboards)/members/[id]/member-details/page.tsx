...
const EfficiencyMemberDetails: React.FC = () => {
  ...
  const individualCurrentMonthPerformance = individualMemberData?.currentMonthScore;
  const individualLastMonthPerformance = individualMemberData?.lastMonthScore;

  const formattedCurrentMonthPerformance =
    individualCurrentMonthPerformance !== undefined ? `${(Math.ceil(individualCurrentMonthPerformance * 100) / 100).toFixed(2)}%` : "-";

  const formattedLastMonthPerformance =
    individualLastMonthPerformance !== undefined ? `${(Math.ceil(individualLastMonthPerformance * 100) / 100).toFixed(2)}%` : "-";
    ...
    <div className="w-full">
      ...
      <div className="pl-4 pt-8 pr-[14px] w-full">
        ...
        <div className="flex justify-between items-center">
          <GlobalBreadCrumb initialData="Members" initalLink="/members" secondayData="Performance" secondayLink="/members/id/member-details" />
        ...
        {individualMemberDataLoading ? (
          ...
          <PageHeading title={`${firstName}'s Performance`} titleclassName="font-medium" className="pl-2 pt-1" />
        )}
              ...
              <>
                <div className="flex flex-row md:gap-x-4 md:gap-y-0 item-start md:items-center mr-4">
                  ...
                  <div className="pl-2 pt-11 md:pt-1 md:pl-0">
                    <h1 className="pb-[3px] text-sm text-[#334155] font-semibold">{individualName}</h1>
                    <p className="text-twelve text-[#334155]">{individualDesignation}</p>
                  </div>
                </div>
                <div className="pl-6 md:pl-6 md:pb-5 lg:pl-0 lg:pt-5 flex gap-x-[14px] md:gap-x-[10px]">
                  <div className="pt-1 md:pt-0">
                    <div className="flex items-center gap-x-[6px]">
                      ...
                      <h1 className="md:text-[18px] font-semibold">{formattedCurrentMonthPerformance}</h1>
                    </div>
                    <p className="text-twelve md:text-sm text-miniSubheadingColor">Current Month Performance</p>
                  </div>
                  <div>
                    <div className="flex items-center gap-x-[6px]">
                      <CalendarArrowDown size={16} />
                      <h1 className="text-[18px] font-semibold">{formattedLastMonthPerformance}</h1>
                    </div>
                    <p className="text-twelve md:text-sm text-miniSubheadingColor">Last Month Performance</p>
                  </div>
                </div>
              </>
            )}
          ...
          <span className="md:hidden pt-14 pr-2 md:pb-0 md:pr-0 relative">
            ...
            {isMenuOpen && (
              <div className="absolute right-0 mt-2 w-48 bg-white border border-gray-200 rounded shadow-lg z-10 modal-content">
                <ul>
                  <li className="px-4 py-2 text-start cursor-not-allowed text-gray-400">View Profile</li>
                  <li className="px-4 py-2 text-start cursor-not-allowed text-gray-400">Edit Profile</li>
                  <hr className="mt-1" />
                  <li className="px-4 py-2 text-destructive hover:bg-gray-100 cursor-pointer">
                    <DisableModal
                      trigger={<span className="text-sm font-semibold">Disable Member</span>}
                      // togglefn={() => {
                      //   handleToogle(id);
                      // }}
                    />
